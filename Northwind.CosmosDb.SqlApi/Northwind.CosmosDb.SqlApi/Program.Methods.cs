...
using Northwind.Shared; // NorthwindContext, Product, Category, and so on
        ...
        try
        {
            ...
            )
            {
                ...
                ContainerProperties containerProperties =
                    new("Products", partitionKeyPath: "/productId")
                    {
                        IndexingPolicy = indexingPolicy
                    };
                ...
                WriteLine(
                    "  IndexingPolicy.IndexingMode: {0}",
                    arg0: properties.IndexingPolicy.IndexingMode
                );
                WriteLine(
                    "  IndexingPolicy.IncludedPaths: {0}",
                    arg0: string.Join(
                        ",",
                        properties.IndexingPolicy.IncludedPaths.Select(path => path.Path)
                    )
                );
            }
        }
                ...
                using (NorthwindContext db = new())
                {
                    ...
                    foreach (ProductCosmos product in products)
                    {
                        try
                        {
                            ...
                            WriteLine(
                                "Item with id: {0} exists. Query consumed {1} RUs.",
                        ...
                        catch (CosmosException ex) when (ex.StatusCode == HttpStatusCode.NotFound)
                        {
                            ...
                            WriteLine(
                                "Created item with id: {0}. Insert consumed {1} RUs.",
                                productResponse.Resource.id,
                                productResponse.RequestCharge
                            );
        ...
        try
        {
            ...
            )
            {
                ...
                while (resultsIterator.HasMoreResults)
                {
                    ...
                    WriteLine(
                        "Status code: {0}, Request charge: {1} RUs.",
                        products.StatusCode,
                        products.RequestCharge
                    );
                    ...
                    {
                        WriteLine(
                            "id: {0}, productName: {1}, unitPrice: {2}",
                            arg0: product.id,
                            arg1: product.productName,
