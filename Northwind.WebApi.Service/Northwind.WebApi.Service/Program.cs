...
if (!useMicrosoftRateLimiting)
{
    using (IServiceScope scope = app.Services.CreateScope())
    {
        IClientPolicyStore clientPolicyStore =
            scope.ServiceProvider.GetRequiredService<IClientPolicyStore>();
...
app.MapGet(
        "/secret",
        (ClaimsPrincipal user) => $"Hello {user.Identity?.Name}. The secret ingredient is love."
    )
...
app.MapGet(
        "api/products",
        ([FromServices] NorthwindContext db, [FromQuery] int? page) =>
            db.Products
                .Where(product => (product.UnitsInStock > 0) && (!product.Discontinued))
...
app.MapGet(
        ...
        ([FromServices] NorthwindContext db) => db.Products.Where(product => product.Discontinued)
    )
...
app.MapPost(
        "api/products",
        async ([FromBody] Product product, [FromServices] NorthwindContext db) =>
        {
...
app.MapDelete(
        "api/products/{id:int}",
        async ([FromRoute] int id, [FromServices] NorthwindContext db) =>
        {
